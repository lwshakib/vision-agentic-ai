...
async function main() {
  ...
  try {
    ...
    console.log(`\u001b[32m[Info] Bucket "${AWS_S3_BUCKET_NAME}" already exists.\u001b[0m`);
  } catch (error: any) {
    if (error.name === "NotFound" || error.$metadata?.httpStatusCode === 404) {
      console.log(`\u001b[33m[Setup] Bucket "${AWS_S3_BUCKET_NAME}" not found. Creating...\u001b[0m`);
      await client.send(new CreateBucketCommand({ Bucket: AWS_S3_BUCKET_NAME }));
      console.log(`\u001b[32m[Success] Bucket "${AWS_S3_BUCKET_NAME}" created successfully.\u001b[0m`);
    } else {
      console.error("\u001b[31m[Error] Checking/Creating bucket:\u001b[0m", error);
      process.exit(1);
    }
  }
  ...
  try {
    ...
    console.warn("\u001b[33m[Warning] Could not configure CORS:\u001b[0m", error);
  }
