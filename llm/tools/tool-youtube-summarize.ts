...
export const youtubeSummarizeTool: ToolDefinition = {
  description:
    'Summarizes or answers questions about the content of a YouTube video given its URL.',
  schema: z.object({
    ...
    prompt: z
      ...
      .optional()
      .describe(
        'An optional specific prompt or question about the video. Defaults to "Please summarize the video."',
      ),
  }),
  execute: async ({ url, prompt = 'Please summarize the video.' }) => {
    try {
      ...
      console.error(
        '[tool-youtube-summarize] Error summarizing YouTube video:',
        error,
      );
